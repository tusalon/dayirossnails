...
// ============================================
const NEGOCIO_ID_POR_DEFECTO = 'f2858b53-0c42-4767-9ce1-51abf22143a6'; // ID de Dayi-Ross-Nails
...
window.getNombreNegocio = async function() {
    ...
    return config?.nombre || 'Dayi-Ross-Nails';
};
...
window.getTelefonoDuenno = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.telefono || '54374624';
};
...
window.getEmailNegocio = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.email || 'dayelisdelarosa03@icloud.com';
};
...
window.getMensajeBienvenida = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.mensaje_bienvenida || '¡Bienvenida a Dayi-Ross-Nails!';
};
...
window.getNtfyTopic = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.ntfy_topic || 'dayirossnails';
};
...
console.log('✅ config-negocio.js listo para Dayi-Ross-Nails');
